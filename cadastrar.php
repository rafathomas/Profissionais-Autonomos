...
if (isset($_POST['cadastrar']) && $_POST['cadastrar'] == 'sim') :
	...
	if (!strstr($novos_campos['email'], '@')) {
		...
		$inserir_banco = $pdo->prepare("INSERT INTO `profissionais` SET nome = ?, usuario = ?, email = ?, telefone = ?,  celular = ?, sexo = ?, senha = ?, senhac= ?, profissao_id = ?,  descricao_profissao = ?, nivel = ?");
		$array_sql = array(
			$novos_campos['nome'],
			...
			$novos_campos['senha'],
			$novos_campos['csenha'],
			$novos_campos['profissao_id'],
			$novos_campos['descricao_profissao'],
			$novos_campos['nivel'],
		);
		if ($inserir_banco->execute($array_sql)) {
			$respostas['erro'] = 'nao';
			$respostas['msg'] = 'Profissional inserido com sucesso!';
		} else {
			$respostas['erro'] = 'sim';
			$respostas['getErro'] = 'Nao foi possível inserir o profissional no banco de dados';
		}
	}
